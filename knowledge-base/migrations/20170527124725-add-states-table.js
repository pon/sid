const states = [
  ...
  {name: 'Wyoming', key: 'WY'},
  {name: 'Armed Forces Europe, the Middle East, and Canada', key: 'AE'},
  {name: 'Armed Forces Pacific', key: 'AP'},
  {name: 'Armed Forces Americas (except Canada)', key: 'AA'}
];

const insertSql = states.reduce((sql, state) => {
  sql += `INSERT INTO states(id, name) VALUES ('${state.key}', '${state.name}');\n`;
  return sql;
}, '');
...
module.exports = {
  up: function (queryInterface, Sequelize) {
    ...
    .then(() => {
      return queryInterface.sequelize.query(insertSql);
    });
  }
}
